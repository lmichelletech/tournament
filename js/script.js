...
function setTeams(teamsArray){
  ...
  for(var i=length; i>(length-8); i--){
    ...
    //Add the element to the teamContainer using InnerHTML and select the element by using the random number to 
    //grab from the teamsArray. Once you are done push the selected element into the temp array and remove the 
    //element from the teamsArray by using the function splice
...
//This simply returns an object of arrays to hold the places of the teams as they go through each round
...
function setSpots(teams){
  //Use a forEach loop to loop over the teams array and pass in a function that calls the toggle function. 
  //The first parameter should be teams[index] and the second parameter should say `spot${index+1}`.
...
//function tochange the players who are playing
...
//when all the games are played then declare the game over
function gameOver(){
    play.disabled = true
...
//set up a new game to be played
